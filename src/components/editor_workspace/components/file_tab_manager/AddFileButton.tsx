...
const AddFileButton: React.FC<Props> = ({ addFile, getFileByName }) => {
  ...
  return (
    <div className="h-full flex items-center">
      <div
        className="w-screen h-screen bg-black bg-opacity-60 fixed z-[1001] top-0 left-0 justify-center items-center"
        style={{ display: isPopUpOpen ? "flex" : "none" }}
      >
        <div className="flex flex-col justify-center items-center bg-main-bg-color p-5 rounded">
          {error != "" && (
            <p className="pb-4 text-sm text-orange-400">{error}</p>
          )}
          ...
          <div className="flex flex-row w-full">
            ...
            <label
              htmlFor="fileInput"
              className="bg-emerald-500 w-[20%] ml-2 rounded p-3 text-center cursor-pointer"
            >
