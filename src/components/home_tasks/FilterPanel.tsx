...
const FilterPanel: React.FC<{ queryParams: QueryParams }> = ({
  queryParams,
}) => {
  ...
  const [minCredits, setMinCredits] = useState<number>(
    queryParams.minCredits ? queryParams.minCredits : 0
  );
  const [maxCredits, setMaxCredits] = useState<number>(
    queryParams.maxCredits ? queryParams.maxCredits : 20
  );
  const [timeLeft, setTimeLeft] = useState<number | null>(
    queryParams.timeLeft ? queryParams.timeLeft : null
  );
  ...
  return (
    <div className="h-full w-[30%]">
      <div
        className={
          "max-md:absolute top-0 left-0 h-full w-full bg-secondary-bg-color px-5 py-4 mb-10 rounded lg:flex lg:flex-col justify-between " +
      ...
      >
        <div className="h-[90%] flex flex-col">
          <button
            onClick={togglePopup}
            className={
              "text-white bg-red-400 w-10 h-10 font-bold text-sm rounded-[50%] self-end mb-5 " +
              (isPopupOpen ? "" : "hidden")
            }
          >
          ...
          <div className="flex flex-col items-center lg:items-start">
            <label className="pb-5 lg:pb-3 text-base font-medium">
              Languages
            </label>
              ...
              {languages.map((language, index) => {
                return (
                  <button
                    ...
                    className={
                      "px-4 py-2 rounded-full mr-2 mb-3 " +
                      (isSelected(language)
          ...
          <div className="flex flex-col border-0 border-t border-[#5c5e68] pt-4 pb-4 lg:justify-center items-center lg:items-stretch">
            ...
            <div className="flex items-center">
              ...
              <FontAwesomeIcon
                ...
                className="mx-5 text-main-lighter"
              />
              <input
                type="number"
                value={maxCredits}
        ...
        <button
          ...
          className="w-full bg-emerald-400 rounded py-3 font-semibold mt-6 mb-1"
        >
          Filter
        </button>
      </div>
